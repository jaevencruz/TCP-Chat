...
void FriendsList (LogInfo *log, char *friends[FRIENDMAX]) {
    ...
    //g_signal_connect(GTK_NOTEBOOK(chat->notebook), "switch-page", G_CALLBACK (GetPage), chat);
...
void Message (GtkWidget *widget, gpointer data) {
    ChatWindow *chat = (ChatWindow*)data; 
    //chat->page = gtk_notebook_get_current_page(GTK_NOTEBOOK (chat->notebook));
    //gtk_notebook_set_current_page (GTK_NOTEBOOK (chat->notebook), chat->page);
    char *message = NULL;
    strcpy(message, gtk_entry_get_text(GTK_ENTRY(chat->entry)));
    int size = strlen(message) + 2;
    chat->message = malloc(size);
    ...
    gtk_text_buffer_insert(buffer, &iter, chat->message, -1);
}
/*
void ServerWindow() {
    GtkWidget *window;
