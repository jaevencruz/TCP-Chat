...
*/
    ...
    g_signal_connect(log->window[0], "destroy", G_CALLBACK(Delete), log);
}
...
void CreateAccount (GtkWidget *widget, gpointer data) {
    ...
    g_signal_connect(log->window[1], "destroy", G_CALLBACK(Delete), log);
}
...
void FriendsList (LogInfo *log, char *friends[FRIENDMAX]) {
    ...
    ChatInfo *chat = g_malloc(sizeof(*chat));
    ...
    g_signal_connect(chat->window, "delete-event", G_CALLBACK(Delete), log);
    ...
    g_signal_connect_swapped (button, "clicked", G_CALLBACK (Delete), log);
...
void GetPage (GtkWidget *widget, gpointer data) {
    ChatInfo *chat = (ChatInfo*)data;
    chat->page = gtk_notebook_get_current_page(GTK_NOTEBOOK (chat->notebook));
}

void Message (GtkWidget *widget, gpointer data) {
    LogInfo *log = (LogInfo*)data;
    //log->flag = "--CHAT";
    //gtk_notebook_set_current_page (GTK_NOTEBOOK (log->chat->notebook), log->chat->page);
    int size = strlen(gtk_entry_get_text(GTK_ENTRY(log->chat->entry)));
    log->protocol = malloc(size);
    strcpy(log->protocol, gtk_entry_get_text(GTK_ENTRY(log->chat->entry)));
    size += strlen(log->username) + 3;
    char *message = malloc(size);
    strcpy(message, log->username);
    strcat(message, ": ");
    strcat(message, log->protocol);
    strcat(message, "\n");
    ...
    gtk_text_view_set_editable(GTK_TEXT_VIEW (log->chat->textview), TRUE);
    GtkTextBuffer *buffer = gtk_text_view_get_buffer(GTK_TEXT_VIEW(log->chat->textview));

    GtkTextIter iter;
    gtk_text_buffer_get_end_iter(buffer, &iter);
    gtk_text_buffer_insert(buffer, &iter, message, -1);
    gtk_text_view_set_editable(GTK_TEXT_VIEW (log->chat->textview), FALSE);
    free(message);
}

void UpdateChat (LogInfo *log, char *message) {
    int size = strlen(message) + strlen(log->username) + 3;
...
void ChatWindow(LogInfo *log, char *friends[FRIENDMAX]) {
    ...
    log->chat = g_malloc(sizeof(log->chat));
   
    gtk_widget_hide(log->window[0]);
    log->chat->window = gtk_window_new(GTK_WINDOW_TOPLEVEL);
    gtk_window_set_position(GTK_WINDOW(log->chat->window), GTK_WIN_POS_CENTER);
    gtk_window_set_title(GTK_WINDOW(log->chat->window), "ChatNChess");
    
    g_signal_connect(log->chat->window, "delete-event", G_CALLBACK(Delete), log);

    gtk_container_set_border_width (GTK_CONTAINER(log->chat->window), 10);
    
    table = gtk_table_new(4, 3, FALSE);
    gtk_container_add (GTK_CONTAINER (log->chat->window), table);

    button = gtk_button_new_with_label ("Chess");
    g_signal_connect (button, "clicked", G_CALLBACK (Chess), log);
    gtk_table_attach(GTK_TABLE(table), button, 0, 1, 0, 1, GTK_FILL | GTK_SHRINK, GTK_FILL | GTK_SHRINK, 5, 5);
    gtk_widget_show (button);

    button = gtk_button_new_with_label ("Chat");
    g_signal_connect (button, "clicked", G_CALLBACK (Chat), log);
    gtk_table_attach(GTK_TABLE(table), button, 0, 1, 1, 2, GTK_FILL | GTK_SHRINK, GTK_FILL | GTK_SHRINK, 5, 5);
    gtk_widget_show (button);

    button = gtk_button_new_with_label ("Add Friend");
    g_signal_connect (button, "clicked", G_CALLBACK (Add), log);
    gtk_table_attach(GTK_TABLE(table), button, 2, 3, 0, 1, GTK_FILL | GTK_SHRINK, GTK_FILL | GTK_SHRINK, 5, 5);
    gtk_widget_show (button);

    button = gtk_button_new_with_label ("Remove Friend");
    g_signal_connect (button, "clicked", G_CALLBACK (Remove), log);
    gtk_table_attach(GTK_TABLE(table), button, 2, 3, 1, 2, GTK_FILL | GTK_SHRINK, GTK_FILL | GTK_SHRINK, 5, 5);
    gtk_widget_show (button);

    /* Create a new notebook, place the position of the tabs */
    log->chat->notebook = gtk_notebook_new ();
    gtk_notebook_set_tab_pos (GTK_NOTEBOOK (log->chat->notebook), GTK_POS_TOP);
    gtk_table_attach_defaults (GTK_TABLE (table), log->chat->notebook, 0, 3, 2, 3);
    gtk_widget_show (log->chat->notebook);    
    
    sprintf(bufferl, "Global");
    label = gtk_label_new (bufferl);
    frame = gtk_frame_new (NULL);
    gtk_container_set_border_width (GTK_CONTAINER (frame), 10);
    gtk_widget_set_size_request (frame, 480, 320);
    ...
    log->chat->textview = gtk_text_view_new ();
    gtk_text_view_set_editable(GTK_TEXT_VIEW (log->chat->textview), FALSE);
    gtk_container_add (GTK_CONTAINER (frame), log->chat->textview);
    gtk_widget_show (log->chat->textview);

    gtk_notebook_append_page (GTK_NOTEBOOK (log->chat->notebook), frame, label);

    /* Create a button and an entry */
    log->chat->entry = gtk_entry_new();
    gtk_table_attach(GTK_TABLE(table), log->chat->entry, 0, 2, 3, 4, GTK_FILL | GTK_SHRINK, GTK_FILL | GTK_SHRINK, 5, 5);
    gtk_widget_show (log->chat->entry);

    button = gtk_button_new_with_label ("Send");
    g_signal_connect (button, "clicked", G_CALLBACK (Message), log);
    gtk_table_attach(GTK_TABLE(table), button, 2, 3, 3, 4, GTK_FILL | GTK_SHRINK, GTK_FILL | GTK_SHRINK, 5, 5);
    gtk_widget_show (button);

    gtk_widget_show (table);
    gtk_widget_show (log->chat->window);
}
...
void ServerWindow(char *message) {
    GtkWidget *window;  
    // Containers 
    GtkWidget *table;
    GtkWidget *frame;
    ...
    gtk_frame_set_shadow_type(GTK_FRAME(frame), GTK_SHADOW_ETCHED_IN);
    gtk_container_set_border_width (GTK_CONTAINER (frame), 10);
    gtk_table_attach(GTK_TABLE(table), frame, 0, 3, 1, 4, GTK_FILL | GTK_SHRINK, GTK_FILL | GTK_SHRINK, 5, 5);
    
    // SERVER 
    label = gtk_label_new("Server Window");
    pfd = pango_font_description_new();
    pango_font_description_set_size(pfd, 20 * PANGO_SCALE);
    gtk_widget_modify_font(label, pfd);
    gtk_table_attach(GTK_TABLE(table), label, 1, 2, 0, 1, GTK_FILL | GTK_SHRINK, GTK_FILL | GTK_SHRINK, 5, 5);
    
    // TEXT VIEWER
    textview = gtk_text_view_new ();
    gtk_text_view_set_editable(GTK_TEXT_VIEW (textview), TRUE);
    gtk_widget_set_size_request (textview, 480, 320);
    
    int size = strlen(message) + 1;
    char *newMessage = malloc(size);
    strcpy(newMessage, message);
    strcat(newMessage, "\n");
    gtk_table_attach(GTK_TABLE(table), textview, 0, 3, 1, 3, GTK_FILL | GTK_SHRINK, GTK_FILL | GTK_SHRINK, 5, 5);
    ...
    gtk_table_attach(GTK_TABLE(table), button, 0, 3, 3, 4, GTK_FILL | GTK_SHRINK, GTK_FILL | GTK_SHRINK, 5, 5);
